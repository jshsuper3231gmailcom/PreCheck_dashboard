...
| 검색어 | 텍스트 입력 | LOG_ID/분석메세지 통합 검색(OR), 부분일치, 입력 중 자동검색(디바운스 400ms), 검색버튼 없음 (2026-07-28 추가) |

> **[2026-07-29 추가] 상태분류 완전 제외 대상**
> `precheck.detail-exclude-log-ids`(application.yml)에 등록된 LOG_ID(현재 `UC_TOTAL_COUNT`/`UC_HTS_COUNT`/`UC_MTS_COUNT`)는 [에러/경고]·[정상/정보/미분석] 두 탭 어디에도 노출되지 않고, 상단 요약 5개 카운트(에러/경고/정상/정보/미분석) 집계에서도 제외된다. 실시간 접속자수 스파크라인(4-9 하단 참조, `UC_TOTAL_COUNT` 등)과 3번 주요 데이터 카드(`precheck.info-data`)는 이 제외 목록과 무관하게 그대로 표시된다 — 두 기능이 참조하는 쿼리(`selectUcSparkData`, `selectInfoData`)는 LOG_ID를 직접 지정해 조회하는 별도 쿼리이기 때문이다.
...
WHERE ANALYZE_DATE  = #{today}
  ...
  AND (UPPER(LOG_ID) LIKE UPPER('%' || #{keyword} || '%') OR UPPER(ANALYZE_MESSAGE) LIKE UPPER('%' || #{keyword} || '%'))  -- keyword 있을 때만, LOG_ID/분석메세지 통합 부분일치 검색 (2026-07-28 추가)
  AND LOG_ID NOT IN (#{excludeLogIds})  -- precheck.detail-exclude-log-ids 대상은 항상 제외, 리스트 비어있으면 조건 생략 (2026-07-29 추가)
ORDER BY
...
WHERE ANALYZE_DATE  = #{today}
  AND ANALYZE_LEVEL IN ('에러', '경고')
  AND SERVER_ID     = #{serverId}
  AND (UPPER(LOG_ID) LIKE UPPER('%' || #{keyword} || '%') OR UPPER(ANALYZE_MESSAGE) LIKE UPPER('%' || #{keyword} || '%'))  -- keyword 있을 때만 (2026-07-28 추가)
  AND LOG_ID NOT IN (#{excludeLogIds});  -- precheck.detail-exclude-log-ids 대상 제외 (2026-07-29 추가)
...
| `keyword` | LOG_ID/분석메세지 통합 검색어 (부분일치, 대소문자 무시) | 없음 (조건 없음) | error-list, normal-list | `UC_TOTAL` (2026-07-28 추가) |
